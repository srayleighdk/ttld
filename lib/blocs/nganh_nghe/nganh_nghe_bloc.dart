...
class NganhNgheBloc extends Bloc<NganhNgheEvent, NganhNgheState> {
  ...
  NganhNgheBloc({required this.nganhNgheRepository})
      : super(NganhNgheInitial()) {
  ...
  Future<void> _onLoadNganhNghes(
      LoadNganhNghes event, Emitter<NganhNgheState> emit) async {
  ...
  Future<void> _onAddNganhNghe(
      AddNganhNghe event, Emitter<NganhNgheState> emit) async {
    try {
      final nganhNghe = await nganhNgheRepository.addNganhNghe(event.nganhNghe);
      if (state is NganhNgheLoaded) {
        final updatedNganhNghes =
            List<NganhNgheKT>.from((state as NganhNgheLoaded).nganhNghes)
              ..add(nganhNghe);
  ...
  Future<void> _onUpdateNganhNghe(
      UpdateNganhNghe event, Emitter<NganhNgheState> emit) async {
    try {
      final nganhNghe =
          await nganhNgheRepository.updateNganhNghe(event.nganhNghe);
      if (state is NganhNgheLoaded) {
        final updatedNganhNghes = (state as NganhNgheLoaded)
            .nganhNghes
            .map((q) => q.id == nganhNghe.id ? nganhNghe : q)
            .toList();
        emit(NganhNgheLoaded(nganhNghes: updatedNganhNghes));
      }
    } catch (e) {
  ...
  Future<void> _onDeleteNganhNghe(
      DeleteNganhNghe event, Emitter<NganhNgheState> emit) async {
    try {
